...
#strip strings
b = keys[0].tolist()
a = list(map(lambda x: x.replace(x, x+" ").replace(' to ',' ').replace(' at ',' ').replace(' of ',' ').replace(' for ',' ')
             .replace(' on ',' ').replace(' and/or ', ' ').replace(' or ', ' ').replace(' and ',' ').replace(' the ',' ').replace(' The ',' ')
             .replace(' their ',' ').replace(' from ',' ').replace(' in ',' ').replace(' all ',' ').replace(' by ',' ').replace(' a ',' ')
...
#because indicator has one key word more we use the indicator keywords.
...
a = target_des['Description'].tolist()
a = [element.lower() for element in a]
target_lower = pd.DataFrame({'Description':a})
del target_des['Description']
frames = [target_des, target_lower]
target_des = pd.concat(frames, axis=1)
...
a = indicator_des['Description'].tolist()
a = [element.lower() for element in a]
indicator_lower = pd.DataFrame({'Description':a})
del indicator_des['Description']
frames = [target_des, indicator_lower]
indicator_des = pd.concat(frames, axis=1)
...
#     res.append(keys2.applymap(lambda x : " "+x.lower() in a.lower()))

for a in indicator_des['Description']:
    res.append(strip2.applymap(lambda x: np.sum([" "+i+" " in a for i in x.split(' ')]) > 0))
